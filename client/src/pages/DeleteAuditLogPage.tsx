...
import { fetchDeleteAuditLog, type DeleteAuditAction, type DeleteAuditLogEntry } from "../api/client.js";
import { formatDateTime } from "../lib/format.js";
import { AuditLogIcon, ChevronDownIcon, EmptyIcon, ErrorIcon, RetryIcon } from "../lib/icons.js";

const PAGE_SIZE = 50;

const ACTION_LABELS: Record<DeleteAuditAction, string> = {
...
// "additionsC1" -> "Additions C1", "cascadedChildren" -> "Cascaded Children" — a plain,
// generic humanizer rather than a per-action-type template, since `details`' shape
// varies by action and new fields shouldn't need a matching UI change every time.
...
// Read-only view of every Global-Admin delete/undo action (Capitalization delete,
// Addition undo, Disposal undo, Transfer delete) — see server routes/deleteAuditLog.ts.
export function DeleteAuditLogPage() {
  const [items, setItems] = useState<DeleteAuditLogEntry[]>([]);
  ...
  const [action, setAction] = useState<DeleteAuditAction | "">("");
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetchDeleteAuditLog({
      ...
      setError(err instanceof Error ? err.message : "Could not load the delete log.");
    } finally {
      setLoading(false);
    }
    ...
    <div className="flex h-full flex-col bg-white">
      <div className="border-b border-gray-200 px-6 py-4">
        ...
        <p className="mt-1 max-w-xl text-sm text-gray-500">
          Every Global-Admin delete/undo action — Capitalization delete, Addition undo, Disposal undo, Transfer
          delete — newest first. Read-only.
        </p>

        <div className="mt-4 flex flex-wrap items-end gap-3">
          <div className="flex flex-col gap-1">
            <label htmlFor="dl-far-id" className="text-[11px] font-bold uppercase tracking-wide text-gray-500">
              FAR ID
            </label>
            <input
              id="dl-far-id"
          ...
          <div className="flex flex-col gap-1">
            <label htmlFor="dl-action" className="text-[11px] font-bold uppercase tracking-wide text-gray-500">
              Action
            </label>
            <select
              id="dl-action"
              ...
              value={action}
              onChange={(e) => setAction(e.target.value as DeleteAuditAction | "")}
            >
          ...
          <div className="flex flex-col gap-1">
            <label htmlFor="dl-date-from" className="text-[11px] font-bold uppercase tracking-wide text-gray-500">
              From
            </label>
            <input
              id="dl-date-from"
          ...
          <div className="flex flex-col gap-1">
            <label htmlFor="dl-date-to" className="text-[11px] font-bold uppercase tracking-wide text-gray-500">
              To
            </label>
            <input
              id="dl-date-to"
      ...
      <div className="min-h-0 flex-1 overflow-auto px-6 py-4">
        {loading && items.length === 0 ? (
          ...
          <div className="flex flex-col items-center justify-center gap-2 py-24 text-center">
            ...
            <p className="text-sm font-medium text-gray-600">No delete/undo actions recorded yet.</p>
            <p className="text-xs text-gray-400">
              {hasActiveFilters
                ? "Try widening the filters above."
                : "Actions taken from the delete/undo buttons on the Log tabs will show up here."}
            </p>
          </div>
        ) : (
                    ...
                    <tr className="border-b border-gray-100 odd:bg-white even:bg-gray-50/60">
                      ...
                      <td className="py-2 pr-3 font-medium text-ink">
                        ...
                        {entry.transferId !== null && (
                          <span className="font-normal text-gray-400"> (Transfer #{entry.transferId})</span>
                        )}
                      </td>
                      <td className="py-2 pr-3 text-gray-600">{entry.actorUsername ?? "Unknown user"}</td>
                      <td className="py-2 pr-3 text-gray-600">{entry.reason}</td>
