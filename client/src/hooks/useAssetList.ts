...
export function useAssetList(fy: FySettings | null, filters: AssetFilters) {
  ...
  const loadFirstPage = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetchAssets({ asAt: fy.asAt, ...filters, limit: PAGE_SIZE });
      setItems(res.items);
  ...
  const loadMore = useCallback(async () => {
    ...
    setLoadingMore(true);
    try {
      const res = await fetchAssets({ asAt: fy.asAt, ...filters, limit: PAGE_SIZE, cursor: nextCursor });
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [settingsKey, filters, nextCursor, loadingMore]);
